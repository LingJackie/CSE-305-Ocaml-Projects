...
let split_str_thingy (s : string ): string list = 
   match s with
   | "" -> []
   | ss -> (String.split_on_char ',' ss)
...
let interpreter ( (input : string), (output : string )) : unit =
   ...
   let listy = convert_all_pushes ls_str in
      ...
      let println_op_HELPER(st : string): string list=
         (match st with
         |"" -> [":error:"]
         |s -> if (contains s "-s")  then (file_write_string (removeStringMarker s); []) else [":error:";s] ) in
   ...
   (* let println_op(st : string): string =
      match st with
      |"" -> ""
      |s -> if (String.contains s ',')  then (get_first_str (split_str_thingy s)) else "" *)
